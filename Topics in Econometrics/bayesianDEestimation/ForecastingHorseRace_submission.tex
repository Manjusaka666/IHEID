...
\begin{table}[H]
    ...
    \begin{tabular}{lcl}
        ...
        Minnesota tightness             & $\lambda$          & \shortstack[l]{$\lambda \sim \Gamma(\text{mode}=0.05,\text{sd}=0.2)$\\$\lambda\in[0.001,2.0]$} \\
        Lag decay                       & $\alpha$           & \shortstack[l]{$\alpha \sim \Gamma(\text{mode}=3.0,\text{sd}=0.25)$\\$\alpha\in[1,3]$} \\
        Cross-variable shrinkage        & $\psi$             & $\psi_{ij}=\psi_{ij}^{\text{BVAR}}$                                                   \\
        Sum-of-coefficients prior       & $\mu_{\text{SOC}}$ & \shortstack[l]{$\mu_{\text{SOC}}\sim\Gamma(\text{mode}=1,\text{sd}=1)$\\$\mu_{\text{SOC}}\in[0.01,50]$} \\
        Dummy-initial-observation prior & $\mu_{\text{DIO}}$ & \shortstack[l]{$\mu_{\text{DIO}}\sim\Gamma(\text{mode}=1,\text{sd}=1)$\\$\mu_{\text{DIO}}\in[0.01,50]$} \\
        Hyperparameter sampler          &                    & $\text{MH-in-Gibbs}$                                                                  \\
        \bottomrule
    \end{tabular}
\end{table}
...
Here, ``recursive'' refers to re-estimation of the reduced-form VAR as the sample expands; the analysis does not perform structural shock identification, and variable ordering does not enter forecasting or RMSFE calculations.
...
The ordering across information sets is clear: the Full model has the lowest posterior mean $\lambda$, the Medium model is intermediate, and the Small model is highest. Because $\lambda$ scales prior variances in \eqref{eq:mn_var}, lower $\lambda$ corresponds to tighter shrinkage, so the larger information set is regularized more aggressively throughout the evaluation window. All three series step up around 2008--09 and drift upward afterward, indicating that the data support somewhat looser regularization over time while preserving the relative ordering across models.
