...
# (h)
# Compute H_hat
# X_beta_hat <- as.matrix(dat_1000[, c('price', 'male', 'age', 'clothes_shoes', 'cosmetics', 'food', 'technology')]) %*% beta_hat[-1]
# phi_Xb <- dnorm(X_beta_hat)
# Phi_Xb <- pnorm(X_beta_hat)
# Phi_minus_Xb <- pnorm(-X_beta_hat)
# epsilon <- 1e-16
# Phi_Xb <- pmax(Phi_Xb, epsilon)
# Phi_minus_Xb <- pmax(Phi_minus_Xb, epsilon)
# factor <- as.vector((phi_Xb / (Phi_Xb * Phi_minus_Xb))^2)
# X <- as.matrix(cbind(1, dat_1000[, c('price', 'male', 'age', 'clothes_shoes', 'cosmetics', 'food', 'technology')]))
# H_hat <- t(X) %*% (factor * X) / nrow(dat_1000)
# 
# # Compute the asymptotic variance
# V_hat <- solve(H_hat)
# 
# # Extract the variance of the coefficient on age
# variance_beta_age <- V_hat['age', 'age']
# beta_age_sd <- sqrt(variance_beta_age / nrow(dat_1000))
# 
# # Approximate finite sample distribution
# beta_age_values <- seq(beta_hat['age'] - 4 * beta_age_sd, beta_hat['age'] + 4 * beta_age_sd, length.out = 100)
# beta_age_density <- dnorm(beta_age_values, mean = beta_hat['age'], sd = beta_age_sd)
# 
