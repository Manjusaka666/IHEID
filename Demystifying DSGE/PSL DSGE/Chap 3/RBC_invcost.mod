...
var y c k i h w r z q;
var ln_y ln_c ln_k ln_i ln_h ln_w ln_r ln_z ln_q;
var    e_a   e_g   e_c   e_i;


varexo eta_a eta_g eta_c eta_i;

parameters beta delta alpha sigmaC sigmaL chi gy chi_I
			Yss
			% shocks AR(1) terms
...
sigmaC 	= 1;		%Consumption risk
sigmaL 	= 1; 		%Elasticity of labor
chi_I	= 4;		%Investment Adjustment cost
...
model; 
	...
	e_i*(1-(chi_I/2)*(i/i(-1)-1)^2)*i = k-(1-delta)*k(-1);
	[name='shadow value of capital']
	e_i*q = 1 + e_i*q*(chi_I/2)*( 1 + ( 3*i/i(-1)-4 )*i/i(-1) ) + 1/r*e_i(+1)*(1-delta)*q(+1)*chi_I*(1-i(+1)/i)*(i(+1)/i)^2;
...
stoch_simul(irf=60) ln_y ln_c ln_k ln_i ln_h ln_w ln_r ln_z ln_q;
