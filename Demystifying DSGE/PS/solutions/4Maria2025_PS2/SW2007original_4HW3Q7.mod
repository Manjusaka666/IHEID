...
% This version (4HW3Q7) is for HW3Q7 v2024 ==> with  g = crhog*(g(-1)) + eg + cgy*ea; [= original SW2007]
...
c_nlc c_lc debt t  c_lcf c_nlcf debtf tf% <=== NEW
;    
...
%	      c = (chabb/cgamma)/(1+chabb/cgamma)*c(-1) + (1/(1+chabb/cgamma))*c(+1) +((csigma-1)*cwhlc/(csigma*(1+chabb/cgamma)))*(lab-lab(+1)) - (1-chabb/cgamma)/(csigma*(1+chabb/cgamma))*(r-pinf(+1) + 0*b) +b ;
           // Consumption of Ricardian HH <=== NEW
...
stoch_simul(Tex,irf = 40) y c c_nlc c_lc inve pinf r w k lab;
%----------------------------------------------------------------
