...
%	deltaW      ${\delta_W}$       (long_name='family surplus from employed worker')  // deleted in EB version
%	deltaF      ${\delta_F}$       (long_name='firm surplus from employed worker')  // deleted in EB version
...
varexo  
	...
	inno_eta   ${\varepsilon_{\eta}}$       (long_name='bargaining power shock')
;
parameters 
...
%gamma       = 0.8;  // Calvo wage stickiness  ===> contract duration = 5 months
gamma       = 0; // for Figure 2
wage_index  = 0;    // wage indexation
vtheta      = 0.03; // rate of separation
alp         = 0.99; // labour elasticity of production
phi_R     = 0.85^(1/3);	 // interest rate smoothing
...
#nbar        = 1-ubar;   
#mbar        = vtheta*nbar;  // <======= only vthet
#vbar        = mbar/qbar;
#sbar        = mbar/ubar;
%#sigmam      = mbar*(ubar^xi*vbar^(1-xi))^(-1);  // <======= <====== changed for EB 
#sigmam      = 0.3984; // Table 5
#thetabar    = vbar/ubar;
#mcbar       = (epsilon-1)/epsilon;  // <=======
#xLbar       = mcbar;  // <=======
#Rbar        = 1/bet;  // <======= only bet
#zbar        = ybar/(nbar*hbar^(alp));  // <======= alp
#wbar        = xLbar*zbar*alp*hbar^(alp-1);  // <======= alp
#Phi         = Phi_y*ybar/nbar;
#PsiLbar     = xLbar*zbar*hbar^(alp) - wbar*hbar - Phi;  // <======= alp
#PsiCbar     = (1-mcbar)*ybar;  // <=======
%#Jbar		= 1/(1-bet*(1-vtheta))*PsiLbar;  // <======= vthet and bet and wbar (alp)    <====== changed for EB   

%#deltaFbar   = 1/(1-bet*(1-vtheta)*gamma)*wbar*hbar;  // <======= deleted for EB
#b           = b_wh*wbar*hbar;  // <======= wbar (alp)
%#mrsbar      = (Jbar*eta/(1-bet*(1-vtheta)*gamma)*alp/(alp-1)*hbar*wbar - (1-eta)*deltaFbar/(1-bet*(1-vtheta-sbar))*(wbar*hbar-b))/ (Jbar*hbar*eta/(1-bet*(1-vtheta)*gamma)*(-1)/(1-alp) - (1-eta)*deltaFbar*hbar/(1-bet*(1-vtheta-sbar))*1/(1+vphi)  );  // <======= changed for EB
#mrsbar      = xLbar*zbar*alp*hbar^(alp - 1);
%#deltaWbar   = 1/(1-bet*(1-vtheta)*gamma)*hbar*1/(1-alp)*(-alp*wbar - (-1)*mrsbar);  // <======= deleted for EB
#Deltabar    = 1/(1-bet*(1-vtheta-sbar))*(wbar*hbar - mrsbar*hbar/(1+vphi) - b);  // <======= 
#Jbar		= ((1 - eta)/eta)*Deltabar;  
%#kappa       = qbar*bet*Jbar;  // <======= <====== changed for EB  
#kappa       = 0.1588; // Table 5
#cbar        = ybar - kappa*vbar - Phi*nbar-gbar;  // <=======
#lambdabar   = (cbar*(1-habit))^(-sig);  // <=======
%#kappaL      = mrsbar*lambdabar/hbar^vphi;  // <======= <====== changed for EB  
#kappaL      = 310.24; // Table 5
// - factor of proportionality between period profits and wages
#Afactor =  (1-alp)/alp*wbar*hbar/(PsiLbar);  // <======= alp
// Monetary policy rule 
R         =     ((1-phi_R)*phi_Pi)*Pi(-1) + (1-phi_R)*phi_y*y + phi_R*R(-1) + emoney;  									
// consumption Euler equation
lambda    =    lambda(+1)  + R + eb - Pi(+1);           
// marginal utility of consumption
lambda    =     - sig/(1-habit)*( c - habit*c(-1)  );    
// New Keynesian Phillips curve (with inflation indexation)
Pi        =      price_index/(1+bet*price_index)*Pi(-1) 
                + bet/(1+bet*price_index)*Pi(+1)
                + 1/(1+bet*price_index)*(1-omega)*(1-omega*bet)/omega*(mc);
// marginal cost
mc        =     xL;                            
// new matches
m         =     xi*u   + (1-xi)*v;
...
w         = 	 gamma*( w(-1) - Pi + wage_index*Pi(-1) ) + (1-gamma)*wstar;     
// deltaF (-\partial surplus of firm/\partial wage)
%deltaF    = 	    (1-bet*(1-vtheta)*gamma)*( -alp/(1-alp)*wstar + 1/(1-alp)*(xL + z) )
%        										+ bet*(1-vtheta)*gamma*(   -alp/(1-alp)*(wstar + wage_index*Pi - wstar(+1) - Pi(+1) )
%                                                                           + deltaF(+1) + lambda(+1) - lambda  );                      
// deltaW ( \partial surplus of worker/\partial wage) 
...
%                                + bet*(1-vtheta)*gamma*deltaWbar*( lambda(+1) - lambda + deltaW(+1)  );                                              
// value of firm that resets wage
%Jbar*Jstar =     wbar*hbar/alp*( -alp*wstar + xL + z ) <==== changed for EB
%                + bet*(1-vtheta)*gamma/(1-bet*(1-vtheta)*gamma)*wbar*hbar*( wstar(+1) + Pi(+1) - wstar - wage_index*Pi  )
%        		+ bet*(1-vtheta)*Jbar*( lambda(+1)- lambda + Jstar(+1) );	
Jbar*Jstar =     xLbar*zbar*(hbar^alp)*(xL + z + alp*h) - wbar*hbar*(w + h) 
        		+ bet*(1-vtheta)*Jbar*( lambda(+1)- lambda + Jstar(+1) );					
...
Deltabar*Deltastar =   wbar*hbar*1/(1-alp)*( - alp*wstar + xL + z )
                      ...
                      - bet*Deltabar*sbar*s;   
*/					  
Deltabar*Deltastar =   wbar*hbar*(w + h) - (mrsbar*hbar/(1+vphi))*( (1+vphi)*h - lambda)	
   	              + bet*(1-vtheta-sbar)*Deltabar*(  lambda(+1) - lambda + Deltastar(+1)  )
                      - bet*Deltabar*sbar*s;                                                         
// vacancy posting condition
kappa/qbar*( -q )
            =     bet*gamma/(1-bet*(1-vtheta)*gamma)*wbar*hbar*( wstar(+1) - w - wage_index*Pi + Pi(+1) )
                + bet*Jbar*( lambda(+1) - lambda  + Jstar(+1) );                                                                                                          
// resource constraint
ybar*y    =     cbar*c + gbar*g + vbar*kappa*v + Phi*nbar*n;       
// production
y         =     n + z  + alp*h; 
...
eb        =     rho_eb*eb(-1) +  sig_innoeb*inno_eb;             
// government spending
g         =     rho_g*g(-1) +  g_ ;                   
// monetary policy shock
emoney    =     rho_emoney*emoney(-1) + sig_monpol*interest_;       
// productivity shock
z         =     rho_z*z(-1) +  sig_innoz*inno_z;   
end;
...
%stoch_simul (Tex,irf = 40,relative_irf) y Pi R u v PsiL h w Piw; // 5 years = 60 months
stoch_simul (Tex,irf = 20,relative_irf) y u w Pi; // 5 years = 60 months
