...
var      
  ...
  y_level c_level w_level h_level  // level of the SS
  u_level v_level q_level s_level
  mc_level xL_level
  wh_level cshare_gov_level 
;
...
gamma       = 0;  // No nominal wage rigidity
...
sig_innog  =   0.674;    // std dev of govt spending shock
sig_innoz  =   0.571;    // std dev of productivity shock

// ********* calibration targets 
ybar    = 1;                    // steady state output
hbar    = 0.3333;                // steady state hours worked
...
% ====== Reporting: map deviations back to levels with correct units ======
% 1) Log-linear (use exp)
y_level   = ybar*exp(y);
c_level   = cbar*exp(c);
w_level   = wbar*exp(w);
h_level   = hbar*exp(h);
mc_level  = mcbar*exp(mc);
xL_level  = xLbar*exp(xL);
% 2) Percentage-deviation (use multiplicative 1 + x_hat)
u_level   = ubar*(1 + u);
v_level   = vbar*(1 + v);
q_level   = qbar*(1 + q);
s_level   = sbar*(1 + s);

// handy derived levels for the table
wh_level = w_level*h_level;                   // wage income per worker
cshare_gov_level = gbar;                      //
