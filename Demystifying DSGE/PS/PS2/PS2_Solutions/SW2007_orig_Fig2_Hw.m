%% Use SW Models with parameters in Q3
figs_before = findall(0,'Type','figure');
origVis = get(0,'DefaultFigureVisible'); set(0,'DefaultFigureVisible','off');
dynare usmodel_Q3.mod noclearall
set(0,'DefaultFigureVisible',origVis);          
figs_after  = findall(0,'Type','figure');
dynare_figs = setdiff(figs_after, figs_before);
close(dynare_figs);
...
figure('NumberTitle','off','Name','Output/Hours/Inflation/InterestRate Response_Q3');   
     subplot(4,2,1)
...
%% Use SW Models with parameters in Q5
figs_before = findall(0,'Type','figure');
origVis = get(0,'DefaultFigureVisible'); set(0,'DefaultFigureVisible','off');
dynare usmodel_Q5.mod noclearall
set(0,'DefaultFigureVisible',origVis);          
figs_after  = findall(0,'Type','figure');
dynare_figs = setdiff(figs_after, figs_before);
close(dynare_figs);
...
figure('NumberTitle','off','Name','Output/Hours/Inflation/InterestRate Response_Q5');   
     subplot(4,2,1)
     plot(1:1:periods,Y(2,:),'-k', 1:1:periods,Y(3,:),'-b', 1:1:periods,Y(7,:),'--r', 'LineWidth',1.5)
     title(titles(1,:));
