...
var   labobs robs pinfobs dy dc dinve dw  ewma epinfma  zcapf rkf kf pkf    cf invef yf labf wf rrf mc zcap rk k pk    c inve y lab pinf w r a  b g qs  ms  spinf sw kpf kp  c_nlc c_lc cf_nlc cf_lc t bg;    

% Q6: new state/control variables added for the ROT (rule-of-thumb) & fiscal block:
% c_nlc, cf_nlc: consumption of non-liquidity constrained (Euler-optimizing) households
% c_lc,  cf_lc : consumption of liquidity constrained (rule-of-thumb) households
% t           : taxes (instrument in fiscal rule)
% bg          : public debt (linearized; evolves via gov. budget constraint)

varexo ea eb eg  eqs  em  epinf ew  ;  
 
parameters curvw cgy curvp constelab constepinf constebeta cmaw cmap calfa 
		phi_b phi_g omega % Define new parameters
		czcap cbeta csadjcost ctou csigma chabb ccs cinvs cfc 
		cindw cprobw cindp cprobp csigl clandaw 
		crdpi crpi crdy cry crr 
		crhoa crhoas crhob crhog crhols crhoqs crhoms crhopinf crhow ctrend 
...
// flexible economy
		  ...
		  % Ricardians, non-liquidity constrained
	      cf_nlc = (chabb/cgamma)/(1+chabb/cgamma) * cf_nlc(-1) + (1/(1+chabb/cgamma)) * cf_nlc(+1) +((csigma-1)*cwhlc/(csigma*(1+chabb/cgamma)))*(labf-labf(+1)) - (1-chabb/cgamma)/(csigma*(1+chabb/cgamma))*(rrf+0*b) + b ;
	      
% Flex-price economy: aggregate and ROT consumption
% Separate Ricardians and non-Ricardians
% Aggregate consumption is a share-weighted average of the two types
		  % Rule-of-thumb (liquidity-constrained) households
		  cf_lc = cwhlc*(wf + labf) - (1/ccy)*t ;
		  % Aggregation
          cf = (1-omega)*cf_nlc + omega*cf_lc;
		  % Changed wage equation
...
// === Q6 additions: rule-of-thumb households and fiscal block ===
c = (1-omega)*c_nlc + omega*c_lc ;
c_lc = cwhlc*(w + lab) - (1/ccy)*t ;
t = phi_b*bg(-1) + phi_g*g ;  % fiscal rule, tax
bg = bg(-1) + (g - t); % budget constraint
...
y = ccy*c+ciy*inve+g  +  1*crkky*zcap ;
	      ...
	      a = crhoa*a(-1)  + ea;
	      b = crhob*b(-1) + eb;
		  % Simplified exogenous spending shock equation
          g = eg;
	      qs = crhoqs*qs(-1) + eqs;
	      ms = crhoms*ms(-1) + em;
...
// (disabled)    stoch_simul(irf=20) dy pinfobs robs ;
