...
var   labobs robs pinfobs dy dc dinve dw  ewma epinfma  zcapf rkf kf pkf    cf invef yf labf wf rrf mc zcap rk k pk    c inve y lab pinf w r a  b g qs  ms  spinf sw kpf kp
      // --- New variables for Q6 ---
      c_nlc c_lc           // Consumption: Non-Liquidity-Constrained (nlc) and Liquidity-Constrained (lc)
      cf_nlc cf_lc         // Flexible-price counterparts for consumption
      bg                   // Government debt
      t                    // Lump-sum taxes
      ;   
...
// --- New parameters for Q6 ---
omega     // Share of liquidity-constrained (non-Ricardian) households
phi_b     // Fiscal rule: response of taxes to government debt
phi_g     // Fiscal rule: response of taxes to government spending
;
...
// flexible economy
          ...
          // --- CHANGED: split consumption into Ricardian (cf_nlc) and rule-of-thumb (cf_lc), then aggregate cf
          // Ricardian 
          ...
          // Rule-of-thumb
          cf_lc  = (cwly/ccy)*(wf + labf) - (1/ccy)*t;
          // Aggregation
          cf     = (1-omega)*cf_nlc + omega*cf_lc;
          // Changed wage equation
          wf = csigl*labf +(1/(1-chabb/cgamma))*cf_nlc - (chabb/cgamma)/(1-chabb/cgamma)*cf_nlc(-1);
...
// sticky price - wage economy
          ...
          // --- CHANGED: split consumption into Ricardian (c_nlc) and rule-of-thumb (c_lc), then aggregate c
          // Ricardian
          ...
          // Rule-of-thumb: current after-tax labor income
          c_lc  = (cwly/ccy)*(w + lab) - (1/ccy)*t;
          // Aggregation
          ...
          // --- CHANGED: spending shock simplified to pure exogenous process
          g   = eg;   // (replaces: crhog*g(-1) + eg + cgy*ea)
          // --- NEW (Q6): government budget constraint and tax rule
          bg  = bg(-1) + g - t;                     // budget constraint (shares of SS output)
          t = phi_b*bg(-1) + phi_g*g;               // fiscal rule
...
stoch_simul(TeX, order=1, irf=40) y c inve pinf r w k lab;
