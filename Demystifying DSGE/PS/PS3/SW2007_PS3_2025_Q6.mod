...
% % =====================  SW sample 1966Q1–2004Q4  =====================
% % Obs sheet starts 1965Q1 -> first_obs=5 (1966Q1), nobs=156 quarters
% % --------- Run 1: Kimball curvature ON (curvp=10, curvw=10) ----------
resid; steady; check;

estimation(optim=('MaxIter',200),
           datafile='SW_Data4HW', xls_sheet=Obs,
           mode_compute=1,
           first_obs=5, nobs=156,
           presample=4, lik_init=2, prefilter=0,
           mh_replic=0, mh_nblocks=2, mh_jscale=0.20, mh_drop=0.2, Tex);

% % store Laplace log MDD
% md_kimball = oo_.MarginalDensity.Laplace;

% --------- Run 2: Dixit–Stiglitz (curvp=0, curvw=0) ----------
% flip curvature to zero and re-estimate with identical options
% set_param_value('curvp',0);
% set_param_value('curvw',0);

% % refresh steady/check (parameters changed)
% resid; steady; check;

% % keep same priors/estimation block (no changes to estimated_params)
% estimation(optim=('MaxIter',200),
%            datafile='SW_Data4HW', xls_sheet=Obs,
%            mode_compute=1,
%            first_obs=5, nobs=156,
...
% md_ds = oo_.MarginalDensity.Laplace;
