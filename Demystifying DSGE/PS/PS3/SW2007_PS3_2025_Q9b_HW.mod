...
% ---------- Structural block (diffuse priors) ----------
csadjcost, 4.00, 0.01, 20.0, NORMAL_PDF, 4.00, 4.50;      % phi     ~ N(4, 4.5)
cmaw,      0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;     % mu_w    ~ U[0,1]
cmap,      0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;     % mu_p    ~ U[0,1]
csigma,    1.50, 0.01, 10.0, NORMAL_PDF, 1.50, 1.11;      % sigma_c ~ N(1.50,1.11)
chabb,     0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;     % lambda  ~ U[0,1]
cprobw,    0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;     % xi_w    ~ U[0,1]
csigl,     2.00, 0.01, 20.0, NORMAL_PDF, 2.00, 2.25;      % sigma_l ~ N(2.00,2.25)
cprobp,    0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;     % xi_p    ~ U[0,1]
cindw,     0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;     % iota_w  ~ U[0,1]
cindp,     0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;     % iota_p  ~ U[0,1]
czcap,     0.30, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;     % psi     ~ U[0,1]
cfc,       1.25, 0.10, 3.00, NORMAL_PDF, 1.25, 0.36;      % phi     ~ N(1.25,0.36)
calfa,     0.30, 0.01, 0.99, NORMAL_PDF, 0.30, 0.15;      % alpha   ~ N(0.30,0.15)

 

% ---------- Monetary policy rule ----------
crpi,      1.50, 0.00, 5.00, NORMAL_PDF, 1.50, 0.75;      % r_pi  ~ N(1.50,0.75)
crr,       0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;     % rho   ~ U[0,1]
cry,       0.12, 0.00, 1.00, NORMAL_PDF, 0.12, 0.15;      % r_y   ~ N(0.12,0.15)
crdy,      0.12, 0.00, 1.00, NORMAL_PDF, 0.12, 0.15;      % r_dy  ~ N(0.12,0.15)


% ---------- Trends / steady states ----------
constepinf, 0.62, 0.01, 5.00,   GAMMA_PDF, 0.62, 0.30;    % pi_bar     ~ G(0.62,0.30)̄
constebeta, 0.25, 0.01, 5.00,   GAMMA_PDF, 0.25, 0.10;    % beta_const ~ G(0.25,0.10) 
constelab,  0.00, -10.0, 10.0,  NORMAL_PDF, 0.00, 6.00;   % l_bar      ~ N(0,6)̄
ctrend,     0.40, 0.00, 2.00,   NORMAL_PDF, 0.40, 0.30;   % gamma_bar  ~ N(0.40,0.30)    



% ---------- Shock persistences (diffuse, uniform on [0,1]) ----------
crhoa,   0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;
crhob,   0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;
crhog,   0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;
crhoqs,  0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;
crhoms,  0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;
crhopinf,0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;
crhow,   0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;
cgy,     0.50, 0.00, 1.00, UNIFORM_PDF, 0.00, 1.00;


% ---------- Shock standard deviations (Inverse-Gamma with mean 0.10, df=2) ----------
stderr ea,    0.10, 0.0001, 10.0, INV_GAMMA_PDF, 0.10, 2.00;
stderr eb,    0.10, 0.0001, 10.0, INV_GAMMA_PDF, 0.10, 2.00;
stderr eg,    0.10, 0.0001, 10.0, INV_GAMMA_PDF, 0.10, 2.00;
stderr eqs,   0.10, 0.0001, 10.0, INV_GAMMA_PDF, 0.10, 2.00;
stderr em,    0.10, 0.0001, 10.0, INV_GAMMA_PDF, 0.10, 2.00;
stderr epinf, 0.10, 0.0001, 10.0, INV_GAMMA_PDF, 0.10, 2.00;
stderr ew,    0.10, 0.0001, 10.0, INV_GAMMA_PDF, 0.10, 2.00;
end;

varobs dy dc dinve labobs pinfobs dw robs;
estimation(optim=('MaxIter',200),datafile='SW_Data4HW_Wei.xlsx',xls_sheet='Obs',mode_compute=6,first_obs=41,nobs=160, presample=4,lik_init=2,prefilter=0,mh_replic=20000,mh_nblocks=2,mh_jscale=0.20,mh_drop=0.2,Tex) y c inve pinf r w k lab;
