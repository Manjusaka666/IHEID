% SW2007 Shocks and Frictions in US Business Cycles - Bayesian Approach
var 
...
% Structural
calfa,    0.24,0.01,1.0,        NORMAL_PDF, 0.30, 0.15;         % CHANGED: N(0.30,0.15) alpha
csadjcost,6.3325,2,15,          NORMAL_PDF, 4.00, 4.50;        % CHANGED: N(4.00, 4.50)  φ  (investment adj. cost)
% μ_w, μ_p (MA term coeffs)  -> both diffuse Uniform(0,1); keep bounds
cmaw,     .8936,0.01,.9999,     UNIFORM_PDF, 0.00, 1.00;        % CHANGED: U(0,1)
cmap,     .7652,0.01,.9999,     UNIFORM_PDF, 0.00, 1.00;        % CHANGED: U(0,1)
% σ_c
csigma,   1.2312,0.25,3,        NORMAL_PDF, 1.50, 1.11;         % CHANGED: N(1.50,1.11)
% h (habit)
chabb,    0.7205,0.001,0.99,    UNIFORM_PDF, 0.00, 1.00;        % CHANGED: U(0,1)
% ξ_w
cprobw,   0.7937,0.3,0.95,      UNIFORM_PDF, 0.00, 1.00;        % CHANGED: U(0,1)
% σ_l
csigl,    2.8401,0.25,10,       NORMAL_PDF, 2.00, 2.25;         % CHANGED: N(2.00,2.25)
% ξ_p
cprobp,   0.7813,0.5,0.95,      UNIFORM_PDF, 0.00, 1.00;        % CHANGED: U(0,1)
% ι_w, ι_p
cindw,    0.4425,0.01,0.99,     UNIFORM_PDF, 0.00, 1.00;        % CHANGED: U(0,1)
cindp,    0.3291,0.01,0.99,     UNIFORM_PDF, 0.00, 1.00;        % CHANGED: U(0,1)
% ψ (utilization cost)
czcap,    0.2648,0.01,1,        UNIFORM_PDF, 0.00, 1.00;        % CHANGED: U(0,1)
% Φ (fixed cost share)
cfc,      1.4672,1.0,3,         NORMAL_PDF, 1.25, 0.36;         % CHANGED: N(1.25,0.36)
...
% Trends
constepinf,0.7,0.1,2.0,         GAMMA_PDF,  0.62, 0.30;         % CHANGED: G(0.62,0.30)
constebeta,0.7420,0.01,2.0,     GAMMA_PDF,  0.25, 0.10;         % CHANGED: G(0.25, **0.10** per instruction)
constelab, 1.2918,-10.0,10.0,   NORMAL_PDF, 0.00, 6.00;         % CHANGED: N(0,6)
ctrend,   0.3982,0.1,0.8,       NORMAL_PDF, 0.40, 0.30;         % CHANGED: N(0.40,0.30)
...
% MonPol
% r_π, ρ, r_y, r_Δy
crpi,     1.7985,1.0,3,         NORMAL_PDF, 1.50, 0.75;         % CHANGED: N(1.50,0.75) r_pi
crr,      0.8258,0.5,0.975,     UNIFORM_PDF, 0.00, 1.00;        % CHANGED: U(0,1) rho
cry,      0.0893,0.001,0.5,     NORMAL_PDF, 0.12, 0.15;         % CHANGED: N(0.12,0.15) r_y
crdy,     0.2239,0.001,0.5,     NORMAL_PDF, 0.12, 0.15;         % CHANGED: N(0.12,0.15) r_dy
...
% Persistence
% --- Persistence (all Uniform(0,1)) ---
crhoa,    .9676 ,.01,.9999,     UNIFORM_PDF, 0.00, 1.00;        % rho_a
crhob,    .2703 ,.01,.9999,     UNIFORM_PDF, 0.00, 1.00;        % rho_b
crhog,    .9930 ,.01,.9999,     UNIFORM_PDF, 0.00, 1.00;        % rho_g
crhoqs,   .5724 ,.01,.9999,     UNIFORM_PDF, 0.00, 1.00;        % rho_i
crhoms,   .3    ,.01,.9999,     UNIFORM_PDF, 0.00, 1.00;        % rho_r
crhopinf, .8692 ,.01,.9999,     UNIFORM_PDF, 0.00, 1.00;        % rho_p
crhow,    .9546 ,.001,.9999,    UNIFORM_PDF, 0.00, 1.00;        % rho_w
cgy,      0.05  ,0.01,2.0,      UNIFORM_PDF, 0.00, 1.00;        % rho_ga
...
% -------------------- Full SW window: 1965Q1–2004Q4 --------------------
% Obs starts 1965Q1 => first_obs=1, nobs=160
varobs dy dc dinve labobs pinfobs dw robs;

% ------- Run 1: Diffuse priors, quick mode (Laplace) -------
resid; steady; check;
estimation(optim=('MaxIter',200),
           datafile='SW_Data4HW', xls_sheet=Obs,
           mode_compute=1,                  % standard mode finder
           first_obs=1, nobs=160,
           presample=4, lik_init=2, prefilter=0,
           mh_replic=0, mh_nblocks=2, mh_jscale=0.20, mh_drop=0.2, Tex);

% ------- Run 2: Diffuse priors + RWMH sampler -------
%   • mode_compute=6 activates the internal scale-tuning to hit target acceptance
%     (Dynare tunes the proposal scale every ~500 draws; default target ≈ 1/3).
%   • mh_replic=20000 per your requirement (per chain).
%   • You may increase mh_nblocks for parallel chains when convenient.
%   • Results are saved as usual (posteriors, MDD via Laplace, etc.).
% References: Dynare manual & forum notes on mode_compute=6/RWMH. 
% 
% (If you want to reuse tuned scales across reruns see: _mh_scale.mat, `scale_file`.)
% 
% Recompute steady state (good practice between runs)
% resid; steady; check;

% estimation(optim=('MaxIter',200),
%            datafile='SW_Data4HW', xls_sheet=Obs,
%            mode_compute=6,                  % RWMH-friendly mode routine
%            first_obs=1, nobs=160,
%            presample=4, lik_init=2, prefilter=0,
%            mh_replic=20000, mh_nblocks=2, mh_jscale=0.20, mh_drop=0.2, Tex);

stoch_simul(Tex,irf = 40) y c inve pinf r w k lab;

% TeX bundle for Part (B)
...
if system(['pdflatex -halt-on-error -interaction=batchmode ' M_.fname '_Tex_Binder.tex'])
    error('TeX-File did not compile (Part B).')
end
