...
% ===============================================================
% Q8 Part (A): Great Inflation 1966Q2–1979Q2 (inclusive)
% - Use mode_compute = 5 (csminwel-style) for the mode
% Sheet Obs starts 1965Q1 => first_obs=6, nobs=53
% ===============================================================

% Recompute steady state before estimation
% resid; steady; check;

% estimation(optim=('MaxIter',200),
%            datafile='SW_Data4HW', xls_sheet=Obs,
%            mode_compute=5,          % <== per Q8 spec
%            first_obs=6, nobs=53,    % 1966Q2 .. 1979Q2
...
%     error('TeX-File did not compile (Part A).')
% end

% ===============================================================
% Q8 Part (B): Great Moderation 1984Q1–2004Q4 (inclusive)
% As in SW: same model/priors; new window; (mode_compute can be 1)
% Sheet Obs starts 1965Q1 => first_obs=77, nobs=84
% ===============================================================

% Recompute steady state before re-estimation
resid; steady; check;

estimation(optim=('MaxIter',200),
           datafile='SW_Data4HW', xls_sheet=Obs,
           mode_compute=1,          % standard mode finder is fine here
           first_obs=77, nobs=84,   % 1984Q1 .. 2004Q4
           presample=4, lik_init=2, prefilter=0,
           mh_replic=0, mh_nblocks=2, mh_jscale=0.20, mh_drop=0.2, Tex);
