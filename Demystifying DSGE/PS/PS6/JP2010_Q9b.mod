...
% Domestic variables 
    ...
    de         ${\Delta e}$       (long_name='Change in nominal ER')
%Foreign variables
...
model(linear);
    ...
    % Home Euler Equation  // eq. 14
        c - h*c(-1) = c(+1) - h*c - (1/sigma)*(1-h)*( i - pi(+1) ) + (1/sigma)*(1-h)*( eps_g - eps_g(+1) );
    ...
    % Monetary policy rule // eq. 23 
		i = rho_i*i(-1) + psi_pi*pi + psi_y*y + epsilon_i;  
        %  + psi_dy*(y - y(-1)) + psi_e*de
...
%*************************************************************************
  estimated_params;    
	...
	%  psi_e, normal_pdf, 0.25, 0.13;
     ...
     dsge_prior_weight, inv_gamma1_pdf, 1, inf;
        
     stderr epsilon_a, inv_gamma1_pdf, 1, inf;        
     stderr epsilon_i, inv_gamma1_pdf, 1, inf;
	 stderr epsilon_cp, inv_gamma1_pdf, 1, inf;      
     stderr epsilon_rp, inv_gamma1_pdf, 1, inf;       
     stderr epsilon_g, inv_gamma1_pdf, 1, inf;     
     stderr epsilon_gs, inv_gamma1_pdf, 1, inf;     
     stderr epsilon_astar, inv_gamma1_pdf, 1, inf;   
     stderr epsilon_istar, inv_gamma1_pdf, 1, inf;
...
% ESTIMATION  
%*************************************************************************
estimation(Tex,datafile='JP_Australian_Data.csv', first_obs=10,prefilter=1,mode_compute=1,mh_replic=0,dsge_var,dsge_varlag=8) y_au pi_au i_au q_au;

%*************************************************************************
% SIMULATION  
%*************************************************************************
stoch_simul(irf=24) y c i pi pi_h pi_f de nfa mc; 
