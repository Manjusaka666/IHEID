...
varexo 
    ...
    % epsilon_pistar ${\varepsilon_{\pi*}}$  (long_name='foreign inflation shock')
;
...
% Foreign AR(1) initial values with foreign parameters
rho_y_star = 0.5;
rho_pi_star = 0.8;
rho_i_star = 0.9;
...
model(linear);
    ...
    % Monetary policy rule // eq. 23 
		i = rho_i*i(-1) + psi_pi*pi + psi_y*y + psi_dy*(y - y(-1)) + psi_e*de + epsilon_i;  
% Foreign sector as in Monacelli 2005		
    ...
    % --- Foreign block replaced by three AR(1) ---
        y_star  = rho_y_star * y_star(-1)  + epsilon_astar;
        pi_star = rho_pi_star * pi_star(-1) + epsilon_gs;
        % i_star  = rho_i_star * i_star(-1)  + epsilon_istar;
...
%*************************************************************************
  estimated_params;    
	...
	%  psi_istar, gamma_pdf, 0.25, 0.13;
     ...
     rho_y_star, beta_pdf, 0.9, 0.05;
     rho_pi_star, beta_pdf, 0.8, 0.1;
...
% ESTIMATION  
%*************************************************************************
estimation(Tex,datafile='JP_Australian_Data.csv', first_obs=9,prefilter=1,presample=4,mode_compute=4,mh_replic=0) y_au pi_au i_au q_au;
