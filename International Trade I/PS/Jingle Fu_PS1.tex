...
\begin{solution}[Two-by-Two-by-Two with Fixed Coefficients]
    ...
    \begin{enumerate}
        ...
        \item[3.] Consumers have Leontief preferences (they want to consume 1 Bun and 1 Sausage per hotdog). Because the consumption ratio is fixed at 1, autarkic equilibrium production must satisfy $B=S$.
                ...
                \item[(a)] We first find the ebalanced production point in both countries:
                    ...
                    The zero-profit conditions then are:
                        \begin{align*}
                            ...
                            P_{SG} &= 2W
                        \end{align*}
                    These relative prices are in line with our prediction from part (1). For Austria $\frac{P_{BA}}{P_{SA}} = 3 > \frac{1}{2} = \frac{P_{BG}}{P_{SG}}$,
                    and it exports sausages, so this is in line with the Heckscher-Ohlin theorem.
                    \begin{center}
                        \begin{tikzpicture}[scale=0.8]
                            % First graph (Country A)
                            \begin{scope}
                                \draw[->] (0,0) -- (5.5,0) node[right] {$Q_{BA}$};
                                \draw[->] (0,0) -- (0,6) node[above] {$Q_{SA}$};
                                \draw[red, thick] plot[domain=0:5, samples=100] (\x, {5/2 - \x/2});
                                ...
                                \draw[orange, thick, dashed] (0,0) -- (5,5);
                                ...
                                \node[above] at (2.5,6) {Country A};
                            \end{scope}
                            
                            % Second graph (Country G) - shifted to the right
            ...
            \item[4.]
                \begin{enumerate}
                    \item[(a)] The production of both countries will be at the kink point where both factors are fully employed:
                    In Austria: $(Q_B, Q_S) = (100, 160)$, and in Germany: $(Q_B, Q_S) = (240, 180)$.
