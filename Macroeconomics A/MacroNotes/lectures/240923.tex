...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[>=stealth, scale=1.2]
        ...
        % Draw the curves
        \draw[thick,domain=0:6,samples=100,smooth,variable=\x] plot ({\x},{4*\x/(0.5+\x) + 0.4/4.5*\x}) node[right] {$f(k_t)$};
        \draw[thick,domain=0:6,samples=100,smooth,variable=\x] plot ({\x},{3.2*\x/(0.5+\x) + 0.32/4.5*\x}) node[right] {$sf(k_t)$};
        \draw[thick,domain=0:6.2] plot ({\x},{3.2/4.5*\x}) node[right] {$\delta k_t$};
        ...
        \node[draw=red, thick, fill=white] at (6.4,6.5) {\scriptsize \textcolor{blue}{Total depreciation}};
